...
for /f "usebackq skip=2 tokens=1" $i in (`curl http://uat-api-rbac.oneplanets.com/version`) do (
  export  aad=$i
  goto :ebd
  )
...
for /f "usebackq skip=1 tokens=1" $i in (`curl http://uat-ebd-connect.oneplanets.com/version`) do (
  export  ebd=$i
  goto :invoice_api
  )
...
for /f "usebackq skip=1 tokens=1" $i in (`curl http://uat-api-invoice.oneplanets.com/version`) do (
...
for /f "usebackq skip=2 tokens=1" $i in (`curl http://uat-invoice-portal.oneplanets.com/version`) do (
...
for /f "usebackq skip=1 tokens=1" $i in (`curl http://uat-api-sap.oneplanets.com/version`) do (
...
for /f "usebackq skip=1 tokens=1" $i in (`curl http://uat-search-api.oneplanets.com/version`) do (
...
@echo *** Invoice On UAT Version ***  >> .\$path_daysof$\SUBFILENAME.txt 
