...
*** Test Cases ***
TC_eINVOICE_01114 - Verify action in Manage invoice page view detail in specific type status and user permission with account True App Type
    ...
    Then Search invoice specific type and status with ${TRUE_APP_TYPE_1} and "${TRUE_CODE}" then view detail to verify action displayed correctly

TC_eINVOICE_01114 - Verify action in Manage invoice page view detail in specific type status and user permission with account True View
    [Tags]      regression      high    web            
    Given Go to eInvoice via ep "${TRUE_EP_WEB}" site with "${TRUE_VIEW_1}" and "${TRUE_PASSWORD}"    
    When Go to Manage invoice page    
    Then Search invoice specific type and status with ${TRUE_VIEW_1} and "${TRUE_CODE}" then view detail to verify action displayed correctly
...
#Search invoice specific type and status with ${user_name} and "${site_code}" then view detail to verify action displayed correctly    
#    ${jsonfile}  ${users}  ${user_index}=  Get user data in json append to list from    ${user_name}    ${jason_buyer_path}    ${site_code}    
#    Loop for select status and search invoice by type then view detail and verify action      ${site_code}    ${jsonfile}    ${users}    ${user_index}   
